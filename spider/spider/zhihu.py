...
class ZhihuSpider(scrapy.Spider):
    ...
    def parse(self, json_data):
        ...
        if flag:
            ...
            # yield scrapy.Request(url=next_url, dont_filter=True,callback=self.parse, meta={'json_data': json_data})
        # if (len(next_url)!=0 and (is_end =="false" or is_end == "False")):
        #     for url in new_urls:
        #         self.driver.get(url)
        #         time.sleep(3)
        #         json_data = self.driver.page_source
        #         yield scrapy.Request(url, callback=self.parse, meta={'json_data': json_data})
